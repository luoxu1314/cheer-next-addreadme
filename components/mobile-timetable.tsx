                  ...
                  >
                    {coursesInThisSlot.length > 0 && (
                      <CourseClickWrapper key={coursesInThisSlot[0].seq} course={coursesInThisSlot[0]} courses={coursesInThisSlot}>
                        <div className={`h-full bg-gradient-to-br from-chart-1/10 to-chart-3/10 rounded-lg border border-border/50 p-1.5 hover:shadow-md transition-all duration-200 ${coursesInThisSlot.length > 1 ? 'relative' : ''}`}>
                          <div className="space-y-1">
                            ...
                            <div className="space-y-0.5">
                              <div className="flex items-center gap-1 text-[9px] text-muted-foreground">
                                ...
                                <span className="truncate">{coursesInThisSlot[0].teachers[0]?.name || ''}</span>
                              </div>
                              <div className="flex items-center gap-1 text-[9px] text-muted-foreground">
                                <MapPin className="w-2 h-2 text-chart-3" />
                                <span className="truncate">{coursesInThisSlot[0].location.name}</span>
                              </div>
                            </div>
